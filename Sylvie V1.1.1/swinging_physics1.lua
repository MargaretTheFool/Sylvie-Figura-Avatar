-- Swinging Physics by Manuel_ | Modified by DeadVoxels to be used for her Sylvie avatar's ribbon physics
...
function SwingingPhysics.swingOnHead(part, dir, limits, root, depth)
    ...
    local fric = friction*math.pow(1.2, depth)
    local handler = {
        ---@type SwingHandler
    }
    ...
    function events.tick()
        --if not handler.enabled then return log("handler disabled") end
        ...
        local zRotOut = rot.z
        rot.z = (rot.z + rot.z + rot.y) / 3
        local RelativeXVel = vectors.rotateAroundAxis(player:getRot().y, player:getVelocity(), player:getRot().xy_).x
        if tostring(part:getName():find("Ribbon1")) == '1' then
            rot.y = (rot.y + rot.y + zRotOut + -(rot.x * RelativeXVel * 2)) / 4
        else
            rot.y = (rot.y + rot.y + zRotOut + (rot.x * RelativeXVel * 2)) / 4
        end
    end
