...
function events.tick()
  ...
  if dying and not inDeathAnimation then
    ...
    local deathSound = sounds["minecraft:entity.fox.death"]
      :volume(1)
      :setPitch(1)
      :subtitle("Sylvie ded :<")
      :setPos(player:getPos())
      :play()
    familyGuyDeath = math.random(1, 5) == 1
    if familyGuyDeath then
      animations.Sylvie.familyguy:play()
      models.Sylvie:setRot(0, player:getRot().y - client:getCameraRot().y - 90, 0)
    end
...
function events.render(delta)
  local death_time = player:getDeathTime()
  if death_time > 0 and familyGuyDeath then
    local progress = math.sqrt((death_time + delta - 1) * 0.08--[[/ 20 * 1.6]])
    if progress > 1 then progress = 1 end
    models.Sylvie:setRot(0, player:getBodyYaw() - client:getCameraRot().y - 90, -(progress * 90))
  else
    models.Sylvie:setRot()
  end
end
